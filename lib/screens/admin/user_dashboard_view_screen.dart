...
class _UserDashboardViewScreenState extends State<UserDashboardViewScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final activeUsers = sampleUsers
        .where((u) => u['status'] == 'Active')
        .length;
    final totalDevices = sampleUsers.fold<int>(
      0,
      (p, u) => p + (u['sensors'] as int),
    );
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            Column(
              children: sampleUsers.map((u) => _buildAdminUserCard(u)).toList(),
            ),
          ],
        ),
      ),
    ...
    return ChangeNotifierProvider<DashboardProvider>(
      ...
      child: Consumer<DashboardProvider>(
        builder: (context, dashboardProvider, child) {
          ...
          return Scaffold(
            appBar: AppBar(
              title: Text(widget.userName ?? 'User Dashboard'),
              backgroundColor: _primaryBlue,
            ),
            body: SingleChildScrollView(
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            'Kolam: ${widget.pondName ?? widget.pondId ?? ''}',
                          ),
                                      ...
                                      children: [
                                        Expanded(
                                          child: _buildGaugeCard(
                                            ...
                                            status: tempStatus,
                                            ranges:
                                                GaugeHelper.getTemperatureRanges(),
                                          ),
                                        ),
                                        const SizedBox(width: 12),
                                        Expanded(
                  ...
                  child: Row(
                    ...
                    children: [
                      _buildMiniGauge(
                        ...
                        status: GaugeHelper.getTemperatureStatus(
                          (u['temperature'] as double),
                        ),
                      ...
                      _buildMiniGauge(
                        ...
                        maxV: 15,
                        ranges: GaugeHelper.getOxygenRanges(),
                        status: GaugeHelper.getOxygenStatus(
                          (u['oxygen'] as double),
                        ),
                ...
                ElevatedButton(
                  onPressed: () {
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (_) => UserDashboardViewScreen(
                          userId: u['uid'],
                          userName: u['name'],
                          pondId: u['pondId'],
                          pondName: u['pond'],
                ...
                TextButton(
                  onPressed: () {
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Text(
                          'Fungsi Jadikan Admin dipanggil untuk ${u['name']}',
                        ),
                      ),
                    );
                  },
          ...
          return Column(
            ...
            children: [
                        ...
                        ClipOval(
                          child: Container(
                            ...
                            decoration: BoxDecoration(
                              ...
                              border: Border.all(
                                color: (color ?? _primaryBlue).withOpacity(
                                  0.08,
                                ),
                                width: 2,
                              ),
              ...
              Text(
                status['status'] ?? '',
                style: TextStyle(
                  color: color ?? status['color'] ?? _primaryBlue,
                  fontSize: 11,
                ),
              ),
            ],
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              status['status'] ?? '',
              style: TextStyle(color: status['color'] ?? _primaryBlue),
            ),
          ],
        ),
      ),
  ...
  Widget _buildPerUserLineChart(DashboardProvider provider) {
    ...
    final data = provider.recentData.isNotEmpty
        ? provider.recentData
        : MockDataGenerator.generateDailyMockData(
            date: DateTime.now(),
            pondId: widget.pondId ?? 'mock',
          );
    // pick field
    final values = <double>[];
    for (final d in data) {
  ...
  Future<void> _generatePDFReport() async {
    try {
      ...
      String fileName =
          'Laporan_${(widget.userName ?? 'user').replaceAll(' ', '_')}_${DateTime.now().millisecondsSinceEpoch}.pdf';
  ...
  Future<void> _generateExcelReport() async {
    try {
      ...
      sheetObject
          .cell(excel_pkg.CellIndex.indexByString('A1'))
          .value = excel_pkg.TextCellValue(
        'Laporan Monitoring Kolam Ikan - ${widget.userName ?? ''}',
      );
      sheetObject.cell(excel_pkg.CellIndex.indexByString('A2')).value =
          excel_pkg.TextCellValue('Kolam: ${widget.pondName ?? ''}');
      sheetObject
          .cell(excel_pkg.CellIndex.indexByString('A3'))
          .value = excel_pkg.TextCellValue(
        'Tanggal: ${DateTime.now().toString().split(' ')[0]}',
      );
      sheetObject.cell(excel_pkg.CellIndex.indexByString('A5')).value =
          excel_pkg.TextCellValue('Waktu');
      sheetObject.cell(excel_pkg.CellIndex.indexByString('B5')).value =
          excel_pkg.TextCellValue('Suhu (°C)');
      sheetObject.cell(excel_pkg.CellIndex.indexByString('C5')).value =
          excel_pkg.TextCellValue('pH');
      sheetObject.cell(excel_pkg.CellIndex.indexByString('D5')).value =
          excel_pkg.TextCellValue('Oksigen (ppm)');
      Random random = Random();
      for (int i = 0; i < 24; i++) {
        int row = i + 6;
        sheetObject
            .cell(excel_pkg.CellIndex.indexByString('A$row'))
            .value = excel_pkg.TextCellValue(
          '${DateTime.now().subtract(Duration(hours: 23 - i)).hour.toString().padLeft(2, '0')}:00',
        );
        sheetObject
            .cell(excel_pkg.CellIndex.indexByString('B$row'))
            .value = excel_pkg.TextCellValue(
          (28 + random.nextDouble() * 4).toStringAsFixed(1),
        );
        sheetObject
            .cell(excel_pkg.CellIndex.indexByString('C$row'))
            .value = excel_pkg.TextCellValue(
          (7.0 + random.nextDouble() * 1.5).toStringAsFixed(1),
        );
        sheetObject
            .cell(excel_pkg.CellIndex.indexByString('D$row'))
            .value = excel_pkg.TextCellValue(
          (6 + random.nextDouble() * 2).toStringAsFixed(1),
        );
      }
      ...
      String fileName =
          'Laporan_${(widget.userName ?? 'user').replaceAll(' ', '_')}_${DateTime.now().millisecondsSinceEpoch}.xlsx';
      final savedPath = await _saveFileBytes(
